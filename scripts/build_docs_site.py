...
# Avoid publishing as security.html — browsers/CDNs often pin stale copies of that name.
NAV = [
    ...
    ("Security.md", "Security", "safe-capture.html"),
]
...
.brand { display: block; text-decoration: none; color: inherit; margin-bottom: 1.5rem; }
.brand-mark {
  font-size: 0.72rem; letter-spacing: 0.14em; text-transform: uppercase;
  color: var(--teal); font-weight: 600;
}
.brand-name {
  display: block; margin-top: 0.35rem; font-size: 1.35rem; font-weight: 700;
...
.content { padding: 2rem 2.25rem 3.5rem; }
article > :first-child { margin-top: 0; }
article h1 {
  font-size: clamp(1.7rem, 3vw, 2.2rem); letter-spacing: -0.035em;
  margin: 0 0 0.85rem; line-height: 1.2;
}
article h2 {
  margin: 2rem 0 0.7rem; font-size: 1.2rem; letter-spacing: -0.02em;
  padding-bottom: 0.35rem; border-bottom: 1px solid var(--line);
}
article h3 { margin: 1.35rem 0 0.45rem; font-size: 1.02rem; color: #dbe7f5; }
...
article table {
  width: 100%; border-collapse: collapse; margin: 1rem 0 1.4rem; font-size: 0.92rem;
...
article th { color: var(--teal); font-weight: 600; background: rgba(45, 212, 191, 0.06); }
article tr:last-child td { border-bottom: 0; }

.codewrap { position: relative; margin: 1rem 0 1.35rem; }
article pre {
  margin: 0; padding: 1rem 1.1rem; overflow: auto; background: var(--code-bg);
  border: 1px solid var(--line); border-radius: var(--radius);
}
article pre code, article code { font-family: var(--mono); font-size: 0.84em; }
article :not(pre) > code {
  background: rgba(45, 212, 191, 0.1);
  border: 1px solid rgba(45, 212, 191, 0.18);
  padding: 0.1em 0.35em; border-radius: 5px; color: #99f6e4;
}
.copybtn {
  position: absolute; top: 0.55rem; right: 0.55rem;
  border: 1px solid var(--line); background: rgba(12, 28, 42, 0.9);
  color: var(--muted); font: 500 0.72rem/1 var(--font);
...
<script src="assets/site.js?v={js_ver}"></script>
</body>
</html>
"""
...
<meta charset="utf-8">
<meta name="color-scheme" content="dark">
<meta http-equiv="refresh" content="0; url=safe-capture.html">
<link rel="canonical" href="safe-capture.html">
<title>Redirecting · Wireshark MCP</title>
<style>
  html,body {{ background:#071018; color:#e8eef7; font-family:system-ui,sans-serif; }}
  a {{ color:#2dd4bf; }}
  main {{ max-width:40rem; margin:4rem auto; padding:0 1rem; }}
</style>
<script>location.replace("safe-capture.html");</script>
</head>
<body>
<main>
  <p>Security docs moved to <a href="safe-capture.html">safe-capture.html</a>.</p>
</main>
</body>
</html>
"""
...
def main() -> None:
    ...
    for src, label, out_name in NAV:
        path = by_name[src]
        body = rewrite_md_links(md_to_html(path.read_text(encoding="utf-8")))
        nav_html = []
    ...
    # Replace stale security.html with a dark redirect (kills old light cached content on refresh)
    (OUT / "security.html").write_text(REDIRECT, encoding="utf-8")
    print(f"wrote {OUT / 'security.html'} (redirect → safe-capture.html)")
    print(f"Built {len(NAV)} pages + assets → {OUT}")
